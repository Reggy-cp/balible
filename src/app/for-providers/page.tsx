...
export default function ForProvidersPage() {
  ...
  return (
    <div style={{ fontFamily: 'var(--font-inter)', backgroundColor: '#F5F1EB', minHeight: '100vh' }}>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden py-20 px-6 lg:px-16 text-center" style={{ backgroundColor: '#111111' }}>
        <div className="max-w-[700px] mx-auto">
          <div className="flex items-center justify-center gap-2 mb-5">
            <Sparkles size={16} style={{ color: '#C8A97E' }} />
            <span style={{ fontSize: 13, color: '#C8A97E', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' }}>For Service Providers</span>
          </div>
          ...
          <a
            href="#apply"
            style={{ display: 'inline-flex', alignItems: 'center', height: 50, padding: '0 32px', backgroundColor: '#C8A97E', color: '#111111', borderRadius: 12, fontSize: 15, fontWeight: 700, textDecoration: 'none' }}
      ...
      <section className="py-14 px-6 lg:px-16 bg-white">
        <div className="max-w-[1000px] mx-auto text-center">
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 28, fontWeight: 700, color: '#111111', marginBottom: 6 }}>What services can you offer?</h2>
          <p style={{ fontSize: 14, color: '#6F675C', marginBottom: 28 }}>Balible accepts providers across 8 categories.</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {CATEGORIES.map(c => (
              <div key={c.label} className="rounded-xl p-4 flex flex-col items-center gap-2" style={{ backgroundColor: '#F5F1EB', border: '1px solid #E8E4DE' }}>
                <span style={{ fontSize: 28 }}>{c.emoji}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#111111', textAlign: 'center' }}>{c.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Benefits */}
      <section className="py-14 px-6 lg:px-16" style={{ backgroundColor: '#F5F1EB' }}>
        <div className="max-w-[1000px] mx-auto">
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 28, fontWeight: 700, color: '#111111', marginBottom: 10, textAlign: 'center' }}>Why join Balible?</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {BENEFITS.map(({ Icon, title, body }) => (
              <div key={title} className="bg-white rounded-xl p-6" style={{ border: '1px solid #E8E4DE' }}>
                <Icon size={26} style={{ color: '#C8A97E', marginBottom: 14 }} strokeWidth={1.5} />
                <h3 style={{ fontSize: 15, fontWeight: 700, color: '#111111', marginBottom: 8 }}>{title}</h3>
      ...
      <section className="py-14 px-6 lg:px-16 bg-white">
        <div className="max-w-[800px] mx-auto">
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 28, fontWeight: 700, color: '#111111', marginBottom: 36, textAlign: 'center' }}>How it works</h2>
          <div className="space-y-6">
            {HOW_IT_WORKS.map(({ step, title, body }) => (
              <div key={step} className="flex items-start gap-5">
                <div className="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: '#111111', fontSize: 15, fontWeight: 700 }}>
                  {step}
                </div>
                <div>
                  <h3 style={{ fontSize: 15, fontWeight: 700, color: '#111111', marginBottom: 4 }}>{title}</h3>
      ...
      {/* Apply form */}
      <section id="apply" className="py-14 px-6 lg:px-16" style={{ backgroundColor: '#F5F1EB' }}>
        <div className="max-w-[560px] mx-auto">
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 28, fontWeight: 700, color: '#111111', marginBottom: 6, textAlign: 'center' }}>Apply to become a provider</h2>
          ...
          {success ? (
            <div className="bg-white rounded-2xl p-10 text-center" style={{ border: '1px solid #E8E4DE' }}>
              <CheckCircle size={40} style={{ color: '#4A7C59', margin: '0 auto 16px' }} />
              <h3 style={{ fontFamily: 'var(--font-playfair)', fontSize: 20, fontWeight: 700, color: '#111111', marginBottom: 8 }}>Application submitted!</h3>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 space-y-5" style={{ border: '1px solid #E8E4DE' }}>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#111111', display: 'block', marginBottom: 6 }}>Business / service name *</label>
                <input
                  value={businessName} onChange={e => setBusinessName(e.target.value)} required
                  placeholder="e.g. Dewi Spa & Beauty"
                  style={{ width: '100%', height: 44, borderRadius: 10, border: '1px solid #E8E4DE', padding: '0 14px', fontSize: 14, color: '#111111', outline: 'none', backgroundColor: '#FAFAF8', fontFamily: 'var(--font-inter)' }}
                />
              </div>

              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#111111', display: 'block', marginBottom: 6 }}>Tell us about your service *</label>
                <textarea
                  value={description} onChange={e => setDescription(e.target.value)} required
                  placeholder="What do you offer? How many years of experience? Where in Bali do you operate?"
                  rows={4}
                  style={{ width: '100%', borderRadius: 10, border: '1px solid #E8E4DE', padding: '12px 14px', fontSize: 14, color: '#111111', outline: 'none', backgroundColor: '#FAFAF8', resize: 'none', fontFamily: 'var(--font-inter)' }}
                />
              </div>
              ...
              <button
                ...
                style={{ height: 50, borderRadius: 12, backgroundColor: '#111111', color: 'white', fontSize: 15, fontWeight: 600, border: 'none', cursor: submitting || !businessName || !description ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-inter)' }}
              >
                {submitting ? 'Submitting…' : 'Submit Application'}
              </button>

              <p style={{ fontSize: 12, color: '#9E9A94', textAlign: 'center' }}>
                You must be signed in to apply. <a href="/sign-in" style={{ color: '#C8A97E' }}>Sign in →</a>
              </p>
            </form>
          )}
        </div>
